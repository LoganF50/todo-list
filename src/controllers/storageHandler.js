...
const StorageHandler = () => {
  ...
  const serializeTodoItem = (todo) => {
    return {
      ...
      dueDate: new Date(todo.getDueDate()),
    };
  };
  ...
  const serializeTodoProject = (project) => {
    ...
    project
      .getTodoList()
      .forEach((todo) => serializedTodos.push(serializeTodoItem(todo)));
    return {
      id: project.getId(),
      title: project.getTitle(),
      todoList: serializedTodos,
    };
  };
  ...
  const getDefaultProject = () => {
    if (localStorage.getItem("defaultProject") === null) {
      ...
      const defaultProject = TodoProject(0, "Default", [
        TodoItem(
          0,
          0,
          "Create first project",
          "hit the + in the sidebar to create your first project",
          newDate
        ),
      ]);
  ...
  const getUserProjects = () => {
    ...
    if (jProjects === null) {
      ...
      jProjects.forEach((project) =>
        projects.push(deserializeTodoProject(project))
      );
      return projects;
    }
  };
  ...
  const setDefaultProject = (defaultProject) => {
    localStorage.setItem(
      "defaultProject",
      JSON.stringify(serializeTodoProject(defaultProject))
    );
  };
  ...
  const setUserProjects = (userProjects) => {
    ...
    userProjects.forEach((project) =>
      storageProjects.push(serializeTodoProject(project))
    );
